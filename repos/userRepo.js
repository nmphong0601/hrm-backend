...
exports.login = function(user_name, password) {
    return new Promise((resolve, reject) => {
        ...
        var sql = `select users.*, json_agg(employees.*) AS "employees" from users inner join employees on users.employee_oid = employees.oid where user_name = '${user_name}' and password = '${md5_password}'`;
        db.load(sql)
            .then(rows => {
                if (rows.length === 0) {
                    resolve(null);
